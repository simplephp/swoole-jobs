        ...
        \Swoole\Process::signal(SIGCHLD, function ($signo) {
            while (true) {
                ...
                if ($ret) {
                    ...
                    $topic = $this->workersInfo[$pid]['topic'] ? $this->workersInfo[$pid]['topic'] : '';
                    $this->status=$this->getMasterData('status');
                    $topicCanNotRestartNum =  $this->dynamicWorkerNum[$topic] ? $this->dynamicWorkerNum[$topic] : 'null';
        ...
        \Swoole\Timer::tick($this->queueTickTimer, function ($timerId) {
            ...
            if ($topics && Process::STATUS_RUNNING == $this->status) {
                ...
                foreach ((array) $topics as  $topic) {
                    ...
                    $this->dynamicWorkerNum[$topic['name']]=$this->dynamicWorkerNum[$topic['name']] ? $this->dynamicWorkerNum[$topic['name']] : 0;
                    $topic['workerMaxNum']                       =$topic['workerMaxNum'] ? $topic['workerMaxNum'] : 0;
                    try {
