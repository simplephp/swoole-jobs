...
class RabbitmqTopicQueue extends BaseTopicQueue
{
    ...
    {
        $this->logger  = $logger;
        $rabbitTopic   = $context->createTopic($exchange ? $exchange : self::EXCHANGE);
    ...
    public static function getConnection(array $config, Logs $logger)
    {
        try {
            ...
            $connection       = new self($context, $config['exchange'] ? $config['exchange'] : null, $logger);
     ...
     */
    public function push($topic, JobObject $job, $delayStrategy=1)
    {
        if (!$this->isConnected()) {
            return '';
        }
        ...
        $delay        = $job->jobExtras['delay'] ? $job->jobExtras['delay'] : 0;
        $priority     = $job->jobExtras['priority'] ? $job->jobExtras['priority'] : BaseTopicQueue::HIGH_LEVEL_1;
        $expiration   = $job->jobExtras['expiration'] ? $job->jobExtras['expiration'] : 0;
        if ($delay > 0) {
